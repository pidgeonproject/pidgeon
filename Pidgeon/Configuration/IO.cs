...
#pragma warning restore
            ...
            {
                foreach (Core.Shortcut RR in Configuration.ShortcutKeylist)
                {
            ...
            config.AppendChild(xmlnode);
            if (Core.Backup(Core.ConfigFile))
                config.Save(Core.ConfigFile);
        ...
        public static bool Load()
        {
            ...
            if (File.Exists(Core.ConfigFile))
            {
                try
                {
                    ...
                    configuration.Load(Core.ConfigFile);
                    lock (Configuration.ShortcutKeylist)
                    {
                        Configuration.ShortcutKeylist = new List<Core.Shortcut>();
                    }
                    lock (Ignoring.IgnoreList)
                    {
                            ...
                            foreach (XmlNode curr in node.ChildNodes)
                            {
                                ...
                                if (curr.Attributes.Count > 0)
                                {
                                    ...
                                    if (curr.Name == "shortcut")
                                    {
                                        if (curr.Attributes.Count > 2)
                                        {
                                            Core.Shortcut list = new Core.Shortcut(Core.ParseKey(curr.Attributes[0].Value), bool.Parse(curr.Attributes[1].Value), bool.Parse(curr.Attributes[2].Value), bool.Parse(curr.Attributes[3].Value));
